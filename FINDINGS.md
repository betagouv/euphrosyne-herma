...
- High: The GUI can crash at startup when the project list is empty or malformed. `DataUploadWidget` checks `projects[0]` even when `projects` may be empty, and the run combo box also unconditionally reads `projects[0]["runs"]`: [data_upload/widget/data_upload.py](/Users/witold/dev/euphrosyne-toolkit/data-upload/data_upload/widget/data_upload.py:69), [data_upload/widget/data_upload.py](/Users/witold/dev/euphrosyne-toolkit/data-upload/data_upload/widget/data_upload.py:93). This is made more likely because `list_projects()` returns `response.json()` without checking HTTP status or shape: [data_upload/euphrosyne/project.py](/Users/witold/dev/euphrosyne-toolkit/data-upload/data_upload/euphrosyne/project.py:27).

- High: Failed uploads are reported as successful. `ProcessWorker` emits the AzCopy return code, but `on_data_upload_completed()` ignores it and always appends `Done.`: [data_upload/app/azcopy.py](/Users/witold/dev/euphrosyne-toolkit/data-upload/data_upload/app/azcopy.py:28), [data_upload/widget/data_upload.py](/Users/witold/dev/euphrosyne-toolkit/data-upload/data_upload/widget/data_upload.py:177). A non-zero AzCopy exit should keep the upload in a failed state and surface the error output.
